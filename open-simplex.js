...
Source: https://github.com/blindman67/SimplexNoiseJS
...
function openSimplexNoise(clientSeed) {
  ...
  const toNums = (s) =>
    s.split(",").map((s) => new Uint8Array(s.split("").map((v) => Number(v))));
  const decode = (m, r, s) =>
    new Int8Array(s.split("").map((v) => parseInt(v, r) + m));
  ...
  var base3D = toNums(
    "0000110010101001,2110210120113111,110010101001211021012011"
  );
  var base4D = toNums(
  ...
  const gradients3D = decode(
    -11,
    23,
    "0ff7mf7fmmfffmfffm07f70f77mm7ff0ff7m0f77m77f0mf7fm7ff0077707770m77f07f70"
  );
    ...
    noise3D(x, y, z) {
      ...
      while (c !== undefined) {
        ...
        if (attn > 0) {
          i =
            pD[
              (((p[(xsb + c.xsb) & 0xff] + (ysb + c.ysb)) & 0xff) +
                (zsb + c.zsb)) &
                0xff
            ];
          attn *= attn;
    ...
    noise4D(x, y, z, w) {
      ...
      while (c !== undefined) {
        ...
        if (attn > 0) {
          ...
          attn *= attn;
          value +=
            attn * attn * (g[i++] * dx + g[i++] * dy + g[i++] * dz + g[i] * dw);
        }
        c = c.next;
      }
